  ...
  chart: function () {
    ...
    function radar(selection) {
      selection.each(function (data) {
        ...
        const getHorizontalPosition = (i, range, factor) =>
          getPosition(i, range, factor, Math.sin);
        const getVerticalPosition = (i, range, factor) =>
          getPosition(i, range, factor, Math.cos);

        // levels && axises
        var levelFactors = d3
          .range(0, cfg.levels)
          .map((level) => radius * ((level + 1) / cfg.levels));
        ...
        data.forEach(function (d) {
          d.axes.forEach(function (axis, i) {
            axis.x = getHorizontalPosition(
              i,
              cfg.w / 2,
              (parseFloat(Math.max(axis.value, 0)) / maxValue) * cfg.factor
            );
            axis.y = getVerticalPosition(
              i,
              cfg.h / 2,
              (parseFloat(Math.max(axis.value, 0)) / maxValue) * cfg.factor
            );
        ...
        if (cfg.circles && cfg.radius) {
          ...
          circle
            ...
            .on("mouseover", function (d) {
              ...
              container.classed("focus", 1);
              container
                .select(".area.radar-chart-serie" + d[1])
                .classed("focused", 1);
            })
